...
int FiveCardDraw::before_turn(Player & p){
	cout << p.name << ": " << p.player_cards << endl;
	...
	while(!valid){
		valid = true;
		discard.clear();
		cout << "Which of your cards will you discard?" << endl;
		string input;
		getline (cin , input);
		stringstream is;
		is << input;
		int count = 0;
		bool dFlag = false;
		vector<int> tempDiscard;
		while(is >> toDiscardS && count<5){
			if(stringstream(toDiscardS) >> toDiscardI){
				dFlag = false;
				count++;
				if(toDiscardI >= 0 && toDiscardI <=4){ 
					for(unsigned int i=0;i<tempDiscard.size();i++){
						if(toDiscardI == tempDiscard[i]){
							dFlag = true;
						}
					}
					if(dFlag == false){
						tempDiscard.push_back(toDiscardI);
						discard.push_back(toDiscardI);
					}
				}
				else{
					valid = false;
	...
	for(int i = discard.size()-1; i>=0; i--){
		discardDeck.add_card(p.player_cards[discard[i]]);
		p.player_cards.remove_card(discard[i]);
	}
...
int FiveCardDraw::turn(Player & p){
	unsigned toDeal = 5 - p.player_cards.size();
	for(unsigned i = 0; i < toDeal; i++){
		if(deck.size() + discardDeck.size() < toDeal){
			return not_enough_cards_error;
		}
		if(deck.size() == 0){
			for(unsigned i = 0; i < discardDeck.size(); i++){
				vector<Card> c = *discardDeck.get_deck_vector();
				deck.add_card(c[i]);
			}
			discardDeck.clear_deck_vector();
			deck.reseed();
			deck.shuffle();
		}
		p.player_cards << deck;
		sort(p.player_cards.hand_vector.begin(),p.player_cards.hand_vector.end());
	}
	return program_success;
}
...
int FiveCardDraw::after_turn(Player & a){
	...
	if(cnt!=players.size()-1){
		cout<<a.name<<": "<<a.player_cards<<endl;
	}
	return program_success;
...
void FiveCardDraw::betting_helper(){
	...
	while(betting){
		//from the first player after dealer to the last player
		for(unsigned n=first; n<players.size(); n++){
			betflag = false;
			//first time bet: bet/call
			if(if_bet == false && (*players[n]).if_fold == false){
				...
				if(betflag==false){
					cout << (*players[n]).name << ": " << (*players[n]).player_cards;
					...
					if(prob_check<33){
						cout << (*players[n]).name << " checks " << endl;
						(*players[n]).playerBet = 0;
						continue;
					}
						...
						bet = 1;
						pot += 1;
						cout << (*players[n]).name << " bets 1 " << endl;
						...
						bet = 2;
						pot += 2;
						cout << (*players[n]).name << " bets 2 " << endl;
				...
				if((*players[n]).playerBet >= (int)bet){
					...
					if(betflag==false){
						...
						if((*players[n]).player_cards.size()==0){
							//if the auto player doesn not get a card, just call
							if((*players[n]).chips < (int)(bet-b)){
								cout << (*players[n]).name << " calls" << endl;
								pot += (*players[n]).chips;
								(*players[n]).chips = 0;
								(*players[n]).playerBet = bet;
							}
							...
							Hand Test_Two_Pair;
							Test_Two_Pair<<*(new Card(Card::card_ranks::two,Card::card_suits::clubs));
							Test_Two_Pair<<*(new Card(Card::card_ranks::two,Card::card_suits::diamonds));
							Test_Two_Pair<<*(new Card(Card::card_ranks::ten,Card::card_suits::clubs));
							Test_Two_Pair<<*(new Card(Card::card_ranks::ten,Card::card_suits::hearts));
							Test_Two_Pair<<*(new Card(Card::card_ranks::nine,Card::card_suits::clubs));
							if(poker_rank((*players[n]).player_cards,Test_Two_Pair)){
								int p=rand()%100;
								int r;
								if(p>50){
									r=1;
								...
								cout << (*players[n]).name << " raises " << r << endl;
								pot += (bet - b) + r;
								bet += r;
								(*players[n]).playerBet = bet;
								(*players[n]).chips -= (bet - b);
		...
		//from the first player to the dealer
	...
	for(unsigned i=0;i<HAND_SIZE;i++){
		for(unsigned j=first; j< players.size(); j ++){
			if(players[j]->if_fold==false){
				...
				sort(players[j]->player_cards.hand_vector.begin(),players[j]->player_cards.hand_vector.end());
			}
		}
		for(int k=0;k<first;k++){
			if(players[k]->if_fold==false){
				players[k]->player_cards<<deck;
				sort(players[k]->player_cards.hand_vector.begin(),players[k]->player_cards.hand_vector.end());
			}
		}
	}
	bool flag=false;
...
void FiveCardDraw::auto_helper(Player & p){
...
int FiveCardDraw::after_round(){
	vector<Player *> *v = new vector<Player *>(players);
	...
	if(cnt==players.size()-1){
		...
		sort(v->begin(),v->end(),poker_rank1);
		(*v)[0]->wins++;
		(*v)[0]->chips+=pot;
	...
	for(unsigned j=0;j<v->size();j++){
		...
		for(unsigned int i=0;i<auto_players.size();i++){
			if((*v)[j]->name==auto_players[i]){
				flag_auto=true;			
			}
		}
	...
	for(unsigned k=0;k<v->size();k++){
		for(unsigned m=0;m<(*v)[k]->player_cards.size();m++){
			deck.add_card((*v)[k]->player_cards[m]);
		}
		for(int m=(int)(*v)[k]->player_cards.size()-1;m>=0;m--){
			(*v)[k]->player_cards.remove_card(m);
		}
	}
	vector<Card> * temp=new vector<Card>;
	temp = discardDeck.get_deck_vector();
	for(int n=(int)discardDeck.size()-1;n>=0;n--){
		deck.add_card((*temp)[n]);
	}
	discardDeck.clear_deck_vector();

	//decide which auto_player leaves the game
	...
	bool saidNo = false;
	while(!saidNo){
		if(players.size()==auto_players.size()){
			for(unsigned i=0; i<players.size();i++){
