...
#pragma warning( push )
#pragma warning( disable : 4482)
//helper method for betting, called before and after cards are dealed
void Razz::betting_helper(){
...
void Razz::auto_helper(Player & p){
...
bool poker_rank4(Player *p2, Player *p1){
	if(p1==0 || p2==0){
		return false;
	}
	else{
		
		Hand h_one=best_hand3(p1);
		Hand h_two=best_hand3(p2);
...
int Razz::after_round(){
	...
	if(cnt==players.size()-1){
		...
		sort(v->begin(),v->end(),poker_rank4);
		(*v)[0]->wins++;
		(*v)[0]->chips+=pot;
