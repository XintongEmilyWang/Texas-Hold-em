...
void SevenCardStud::betting_helper(){
...
int SevenCardStud::before_round(){	
	...
	for(unsigned j=first; j< players.size(); j ++){
		...
		for(unsigned i=0;i<auto_players.size();i++){
			if((*players[j]).name.compare(auto_players[i])==0){
				flag=true;
			}
...
void SevenCardStud::auto_helper(Player & p){
...
bool poker_rank2(Player *p1, Player *p2){
	if(p1==0 || p2==0){
		...
		Hand h_one=best_hand(p1);
		Hand h_two=best_hand(p2);
...
int SevenCardStud::after_round(){
	...
	if(cnt==players.size()-1){
		...
		sort(v->begin(),v->end(),poker_rank2);
		(*v)[0]->wins++;
		(*v)[0]->chips+=pot;
